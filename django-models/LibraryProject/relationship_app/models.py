...
class Library(models.Model):
    ...
    books = models.ManyToManyField(Book, related_name='libraries', blank=True)  # Allows a library to have multiple books
...
class Librarian(models.Model):
    ...
    library = models.OneToOneField(
        Library,  # Uses direct model reference (Django handles order automatically)
        on_delete=models.CASCADE, 
        related_name='librarian'
    )
    
    def __str__(self):
        return f"{self.name} (Librarian at {self.library.name})"  # More descriptive output
